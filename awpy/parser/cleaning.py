...
class DistMetricCallable(Protocol):
    ...
    def __call__(self, *sequences: Sequence[object]) -> float:
        ...
        Args:
            *sequences (Sequence[object]): Sequence to calculate distance between.
...
def _set_distance_metric(metric: str) -> DistMetricCallable:
    """Get the distance metric callable.

    Args:
        metric (str): Identifier for which metric to use

    Returns:
        DistMetricCallable: Desired callable

    Raises:
        ValueError: If an invalid metric is chosen
    """
    if metric == "lcss":
...
def associate_entities(
    ...
    Args:
        game_names (list[str | None] | None): A list of names generated by the demofile.
            Defaults to []
        entity_names (list[str] | None): A list of names: Defaults to []
        metric (Literal['lcss', 'hamming', 'levenshtein', 'jaro', 'difflib']): Distance
            metric, one of lcss, hamming, levenshtein, jaro, difflib.
            Defaults to 'lcss'.

    Returns:
        dict: A dictionary where the keys are entries in game_names,
...
def replace_entities(
    ...
    Args:
        ...
        col_name (str): A column in the Pandas DataFrame
        entity_dict (dict): A dictionary as created in associate_entities()

    Returns:
        pd.DataFrame: A dataframe with replaced names.
