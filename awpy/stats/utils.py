...
def get_rounds_played(player_rounds: pd.DataFrame) -> pd.DataFrame:
    player_round_counts = player_rounds.groupby(["steamid", "side"]).size().reset_index(name="rounds_played")
    total_round_counts = player_round_counts.groupby("steamid")["rounds_played"].sum().reset_index()
