...
def kast(demo: Demo) -> pd.DataFrame:
    ...
    kill_rounds, assist_rounds, trade_rounds, survival_rounds = get_kast_components(kill_df, tick_df)
    all_steamids = get_all_steamids(kill_rounds, assist_rounds, trade_rounds, survival_rounds)

    kast_data = calculate_kast(all_steamids, kill_rounds, assist_rounds, trade_rounds, survival_rounds, tick_df)
    return prepare_kast_df(kast_data)

def get_kast_components(kill_df: pd.DataFrame, tick_df: pd.DataFrame) -> tuple[dict, dict, dict, dict]:
...
def get_survival_rounds(tick_df: pd.DataFrame) -> dict:
    end_ticks = tick_df[tick_df["side"].isin(["t", "ct"])].groupby(["steamid", "round_num"]).tail(1)
...
def calculate_kast(all_steamids: set, kills: dict, assists: dict, trades: dict, survivals: dict, tick_df: pd.DataFrame) -> dict:
    kast = {}
    for steamid in all_steamids:
        total_rounds = calculate_total_rounds_for_side(steamid, "total", tick_df)
        kast[steamid] = {"total": calculate_kast_for_side(steamid, kills, assists, trades, survivals, total_rounds)}
    return kast

def prepare_kast_df(kast_data: dict) -> pd.DataFrame:
    kast_df = pd.DataFrame.from_dict(kast_data, orient="index").reset_index().rename(columns={"index": "steamid"})
    kast_df["side"] = "total"
