...
Get-ChildItem -Path $inputPath -Filter "*.vpk" | Where-Object {
    ...
    # Extract all of the PHYS data from the vmdl_c files
    $physData = ./Source2Viewer-CLI.exe -i $filePath -e "vmdl_c" -f "maps/$fileNameWithoutExtension/entities/" --block "PHYS" 2>&1
