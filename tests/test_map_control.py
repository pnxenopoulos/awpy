...
class TestMapControl:
    ...
    def test_frame_map_control_metric_null_5v0(self):
        ...
        test_mc_metric = frame_map_control_metric(
    ...
    def test_frame_map_control_metric_null_0v0(self):
        """Tests frame_map_control_metric when all players dead."""
        with pytest.raises(ValueError, match="Map not found."):
            frame_map_control_metric(
                map_name="de_mock", frame=self.fake_frames["map_control_null_0v0"]
            )
        with pytest.raises(ValueError, match="players array is length 0"):
            frame_map_control_metric(
                map_name="de_inferno", frame=self.fake_frames["map_control_null_0v0"]
            )
        with pytest.raises(
            ValueError, match="No alive players on either team for given frame"
        ):
            frame_map_control_metric(
                map_name="de_inferno", frame=self.fake_frames["map_control_dead_5v5"]
            )
