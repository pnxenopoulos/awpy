...
class TestConverters:
    ...
    def test_map_hitgroup(self):
        ...
        expected = pd.Series(["generic", "head", "chest", "stomach", "left arm", "right arm", "left leg", "right leg", "neck", "gear", None])
    ...
    def test_map_round_end_reasons(self):
        ...
        expected = pd.Series(["still_in_progress", "target_bombed", "bomb_defused", "ct_win", "t_win", "draw", "hostages_rescued", "target_saved", "hostages_not_rescued", "t_not_escaped", "vip_not_escaped", "game_start", "t_surrender", "ct_surrender", "t_planted", "cts_reached_hostage", None])
        result = map_round_end_reasons(series)
        pd.testing.assert_series_equal(result, expected)

    def test_map_game_phase(self):
        series = pd.Series([0, 1, 2, 3, 4, 5, 6, 7, -1])
        expected = pd.Series(["init", "pregame", "startgame", "preround", "teamwin", "restart", "stalemate", "gameover", None])
