...
class TestParsers:
    ...
    def test_remove_nonplay_ticks_bad_cols(self):
        ...
        with pytest.raises(ValueError, match="is_freeze_period not found in dataframe."):
    ...
    def test_hltv_kills(self, hltv_events: dict[str, pd.DataFrame]):
        ...
        assert hltv_kills[hltv_kills["attacker_team_name"] != hltv_kills["victim_team_name"]].shape[0] == 158
        assert hltv_kills[hltv_kills["attacker_team_name"] != hltv_kills["victim_team_name"]].headshot.sum() == 65
        # Check assists
        hltv_assists = (
            hltv_kills[~hltv_kills["assister_name"].isna() & ~hltv_kills["assistedflash"]]
            .groupby("assister_name")
            .size()
        ).reset_index(name="assists")
        assert all(hltv_assists[hltv_assists["assister_name"] == "Brollan"].assists == 3)
        assert all(hltv_assists[hltv_assists["assister_name"] == "Jimphhat"].assists == 8)
        assert all(hltv_assists[hltv_assists["assister_name"] == "chopper"].assists == 3)
        ...
        assert all(hltv_assists[hltv_assists["assister_name"] == "xertioN"].assists == 7)
    ...
    def test_hltv_damages(self, hltv_events: dict[str, pd.DataFrame]):
        ...
        hltv_damage_total = round(
            hltv_damage[hltv_damage["attacker_team_name"] != hltv_damage["victim_team_name"]]
            .groupby("attacker_name")
            .dmg_health_real.sum()
            / 23,
            1,
        ).reset_index(name="adr")
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "Brollan"].adr == 91.4)
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "Jimpphat"].adr == 87.7)
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "chopper"].adr == 62.1)
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "donk"].adr == 72.8)
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "magixx"].adr == 65.1)
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "sh1ro"].adr == 93.7)
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "siuhy"].adr == 84.7)
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "torzsi"].adr == 73.9)
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "xertioN"].adr == 65.5)
        assert all(hltv_damage_total[hltv_damage_total["attacker_name"] == "zont1x"].adr == 70.8)
    ...
    def test_faceit_kills(self, faceit_events: dict[str, pd.DataFrame]):
        ...
        assert faceit_kills[faceit_kills["attacker_team_name"] != faceit_kills["victim_team_name"]].shape[0] == 156
        assert faceit_kills[faceit_kills["attacker_team_name"] != faceit_kills["victim_team_name"]].headshot.sum() == 90
        # Check assists
        faceit_assists = (
            faceit_kills[~faceit_kills["assister_name"].isna()].groupby("assister_name").size()
        ).reset_index(name="assists")
        assert all(faceit_assists[faceit_assists["assister_name"] == "AMANEK"].assists == 3)
        assert all(faceit_assists[faceit_assists["assister_name"] == "Burmylov"].assists == 2)
        assert all(faceit_assists[faceit_assists["assister_name"] == "Ciocardau"].assists == 7)
        assert all(faceit_assists[faceit_assists["assister_name"] == "LobaIsLove"].assists == 4)
        assert all(faceit_assists[faceit_assists["assister_name"] == "Porya555"].assists == 6)
        assert all(faceit_assists[faceit_assists["assister_name"] == "Sdaim"].assists == 10)
        assert all(faceit_assists[faceit_assists["assister_name"] == "_Aaron"].assists == 4)
        assert all(faceit_assists[faceit_assists["assister_name"] == "bibu_"].assists == 4)
        assert all(faceit_assists[faceit_assists["assister_name"] == "jottAAA-"].assists == 4)
        assert all(faceit_assists[faceit_assists["assister_name"] == "lauNX-"].assists == 5)
