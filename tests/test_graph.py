...
class TestDemoParser:
    ...
    def test_graph_output_graph_dist(self):
        ...
        """
        X, A = frame_to_graph(frame = self.data["GameRounds"][0]["Frames"][0], metric="graph", map_name = self.data["MapName"])
        assert X.shape[0] == 10
        assert X.shape[1] == 10
        assert A.shape[0] == 10
        assert A.shape[1] == 10
    ...
    def test_graph_output_non_graph_dist(self):
        ...
        """
        X, A = frame_to_graph(frame = self.data["GameRounds"][0]["Frames"][0], metric="euclidean", map_name = self.data["MapName"])
        assert X.shape[0] == 10
        assert X.shape[1] == 10
        assert A.shape[0] == 10
        assert A.shape[1] == 10
    ...
    def test_frame_to_graph_bad_map(self):
        ...
        with pytest.raises(ValueError):
            X, A = frame_to_graph(frame = self.data["GameRounds"][0]["Frames"][0], metric="graph", map_name = "not_a_correct_map_name")

    def test_frame_to_graph_no_players(self):
        """ Tests if frame to graph fails on no players in T or CT
        """
        with pytest.raises(ValueError):
            X, A = frame_to_graph(frame = self.data["GameRounds"][-1]["Frames"][-1], metric="graph", map_name = self.data["MapName"])