...
def plot_kills(df, map):
    ...
    for index, row in df.iterrows():
        if row["AttackerSide"] == "T":
            ...
            linecol = "gray"
        plt.scatter(row["AttackerXViz"]+map_adjustments[map][0], row["AttackerYViz"]+map_adjustments[map][1], color = "red", zorder=1)
        plt.scatter(row["VictimXViz"]+map_adjustments[map][0], row["VictimYViz"]+map_adjustments[map][1], color = "blue", zorder=1)
    plt.imshow(map_bg, zorder=0)
    #return plt.show()
